...
inline GLsizei getSize(UBOTypes type, unsigned int arraySize) {
  ...
  case MAT4:
    return 4 * 16;
  case ARRAY_MAT2:
    return arraySize * 2 * 16;
  case ARRAY_MAT3:
    return arraySize * 3 * 16;
  case ARRAY_MAT4:
    return arraySize * 4 * 16;
  default:
    return arraySize * 16;
  }
}
...
public:
  ...
  /*! \brief Creates the buffer (Should be called after the addVariable calls
   * and before map and setVariable) \param bindingPoint The buffer uniform
   * buffer binding point to bind the buffer to
   */
...
private:
   ...
   *  \param arraySize The number of elements in the array if type is a type of
   * array
   */
  void m_setVariable(UBOTypes type, GLsizei offset, GLvoid *data,
                     unsigned int arraySize);

  GLuint m_buffer = 0; //!< The name of the uniform buffer
  std::vector<UBOTypes>
      m_types; //!< The array which stores all types of all variables
  std::map<unsigned int, unsigned int>
      m_arraySizes; //!< The map which stores all array sizes of all array
                    //!< variables
  GLbyte *m_data =
      nullptr; //!< The pointer which is used for writeing to the buffer
...
public:
   ...
   *  \param endifNotThere Defines if a error message should be printed of the
   * uniform wasn't found \return true if the uniform was found and false
   * otherwhise
   */
  ...
  /*! \brief Defines if this Shader is a Shader used for rendering to a
   * ShadowMap3D DEPRECATED \param b The bool value
   */
   ...
   */
  template <class T> void setShaderUpdater();
...
private:
  ...
  std::map<std::string, GLuint>
      m_uniforms; //!< The map where all uniform locations are stored
  ...
  bool m_shadowMap = false; //!< Defines if the Shader is used for rendering to
                            //!< ShadowMap3D DEPRECATED

  std::map<std::string, GLuint>
      m_uniformBlockIndices; //!< The map where every uniform block indices are
                             //!< stored

  ShaderUpdater *m_shaderUpdater = nullptr; //!< The ShaderUpdater of the Shader
};

template <class T> inline void Shader::setShaderUpdater() {
