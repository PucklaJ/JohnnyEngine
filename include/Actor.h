...
public:
  ...
  /*! \brief Gets called after every update method and normally renders the
   * object
   *
   *
   * If the function returns false the application closes
   ...
   *  \param instantAdd if true The child will instantly within this function
   * call added, otherwhise it will be added after the frame \return true if the
   * iniitialisation worked and false if not
   */
   ...
   *  \param del Defines wether the Actor should be deleted when removing from
   * its parent \param instant if true The child will instantly within this
   * function call removed, otherwhise it will be added after the frame
   */
  ...
  /*! \brief Defines wether the position and orientation should change by moving
   * the camera \param b The corresponding bol value
   */
   ...
   *  \param changeInRenderManager if true The Actor will be moved the other
   * Shader inside the RenderManager
   */
...
protected:
  MainClass *m_mainClass = nullptr; //!< The MainClass to which the object or
                                    //!< its parent is attached to
  bool m_isAffectedByCamera = true; //!< Defines if the object should be
                                    //!< transformed depinding on the camera
  bool m_visible = true;            //!< Defines wether the object is visible
  bool m_castsShadows3D =
      true; //!< Defines wether the object casts shadows from a 3D light
  Shader *m_shader = nullptr; //!< The shader whith which the object is rendered

  const char *m_name = ""; //!< The name of the object

private:
  static std::vector<Actor *>
      m_allActors; //!< All actors which are currently not deleted

  /*! \brief Loops through all children (and the children of them) and
   *        sets the shader to RenderManager::DEFAULT_SHADER if the shader is
   * nullptr
   *
   */
  ...
  std::vector<int> m_idsToRemove; //!< Stores all ids of the actors which are
                                  //!< removed with instant = false
  std::vector<bool>
      m_ifDeleteToRemove; //!< Stores all values of del of the actors which are
                          //!< removed with instant = false
  std::vector<Actor *> m_childrenToAdd; //!< Stores all Actors which are added
                                        //!< with instantAdd = false
  static std::vector<int>
      m_ids; //!< Stores all ids of every Actor which isn't deleted
  int m_id;  //!< The ID of the object (unique to every actor which isn't
             //!< deleted)
