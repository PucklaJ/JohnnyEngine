...
public:
   ...
   */
  Physics2D(MainClass *m, const Vector2f &gravity = Vector2f(0.0, -10.0),
            const Vector2f &worldSize = Vector2f(NORM_W, NORM_H));
  virtual ~Physics2D();
  ...
  /*! \brief Set the pixel per metre value which defines how big object in the
   * world is per metre
   *
   */
  ...
  /*! \brief Converts a vector of the pixel coordinates to a vector of the Box2D
   * coordinates \param v The vector to convert \return The coresponding Box2D
   * vector
   */
  b2Vec2 vectorPixelToWorld(const Vector2f &v);
  /*! \brief Converts a vector of the Box2D coordinates to a vector of the pixel
   * coordinates \param v The vector to convert \return The coresponding pixel
   * coordinate vector
   */
  Vector2f vectorWorldToPixel(const b2Vec2 &v);

  /*! \brief Converts a scalar of the pixel coordinates to a scalar of the Box2D
   * coordinates \param s The scalar to convert \return The coresponding scalar
   * of the Box2D coordinates
   */
  float scalarPixelToWorld(float s);
  /*! \brief Converts a scalar of the Box2D coordinates the a scalar of the
   * pixel coordinates \param s The scalar to convert \return The coresponding
   * scalar of the pixel coordinates
   */
   ...
   */
  b2Body *createBOX(const Vector2f &position, float width, float height,
   ...
   */
  b2Body *createBOX(const Vector2f &position, float width, float height,
                    b2BodyDef &bdef, b2FixtureDef &fdef);
   ...
   *  \return The newly created body
   */
  b2Body *createCIRCLE(const Vector2f &position, float radius,
                       float friction = DEF_FRICTION,
                       float restitution = DEF_RESTITUTION,
                       float density = DEF_DENSITY,
                       b2BodyType type = b2_dynamicBody);
   ...
   *  \return The newly created body
   */
  b2Body *createCIRCLE(const Vector2f &position, float radius, b2BodyDef &bdef,
                       b2FixtureDef &fdef);
   ...
   *  \return The newly created body
   */
  b2Body *createCAPSULE(const Vector2f &position, float width, float height,
                        float friction = DEF_FRICTION,
                        float restitution = DEF_RESTITUTION,
                        float density = DEF_DENSITY,
                        b2BodyType type = b2_dynamicBody);
...
private:
  ...
  float m_pixelsPerMetre =
      10.0; //!< The value which defines the size of a body in the physics world
  bool m_debug = false; //!< Defines if it should draw debug graphics
  ContactListener *m_contactListener =
      nullptr; //!< The contact listener which listens to all collisions
};
} // namespace Johnny
