...
public:
   ...
   *  \param format The format of the RenderBuffer in which the bits are
   * interpreted \param target The target to which the render buffer object gets
   * bound \param multisample Defines wether it should be a multisampling buffer
   * or not
   */
  RenderBuffer(GLsizei width, GLsizei height,
               GLenum format = GL_DEPTH24_STENCIL8,
               GLenum target = GL_RENDERBUFFER, bool multisampling = false);
  ~RenderBuffer();
