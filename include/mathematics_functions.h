 ...
 */
template <class T> inline T toRadians(const T &degrees) {
 ...
 */
template <class T> inline T toDegrees(const T &radians) {
 ...
 */
template <class T> inline double abs(const T &d) { return d < (T)0 ? -d : d; }
...
/*! \param percent The amount of percent in which the return value should be
 * true \return true if a random value between 1 and 100 <= percent
 */
inline bool luck(int percent) { return getRand(1, 100) <= percent; }
 ...
 */
template <class T> inline void swap(T var1, T var2) {
  T temp = var1;
  var1 = var2;
  var2 = temp;
}
 ...
 */
template <class T> inline void reverse(const std::vector<T> &vars) {
  int j = 0;
 ...
 */
template <class T> inline int sizeOfArray(T var[]) {
 ...
 */
template <class T> inline T clamp(const T &var, const T &min, const T &max) {
