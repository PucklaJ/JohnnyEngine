...
public:
  ...
  /*! \return The rotation vector of the Transform3D with the rotations on the
   * x-, y- and z-Axis in degrees
   *
   */
  const Vector3f &getRotation() const { return m_rotation; }
  /*! \brief Sets the rotation of the Transform3D
   *  \param v The rotation vector to set with the rotation on the x-,y- and
   * z-Axis in degrees
   */
...
public:
   ...
   */
  virtual void addRotation(GLfloat x, GLfloat y, GLfloat z) {
    setRotation(m_transform.getRotation().x + x,
                m_transform.getRotation().y + y,
                m_transform.getRotation().z + z);
  }
  ...
  virtual void addScale(GLfloat x, GLfloat y, GLfloat z) {
    setScale(getScale().x + x, getScale().y + y, getScale().z + z);
  }
