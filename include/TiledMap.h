...
struct TileAnimation {
  std::vector<TextureRegion>
      regions; //!< The TextureRegions of the key frames on the tileset
...
private:
   ...
   */
  void getSrcRegionForTileID(TextureRegion *srcRegion, int numTilesX, int index,
                             int id);
  ...
  int *m_tilesX = nullptr; //!< An array with all amounts of tiles on the x-Axis
                           //!< of the tilesets
  Colorb m_background;     //!< The background color of the map
  std::vector<b2Body *>
      m_bodies; //!< An array with all bodies loaded with the map
  std::map<int, TileAnimation>
      m_animations; //!< A map which stores all TileAnimations
  std::vector<AnimationTile> m_animTiles; //!< An array with all animated tiles
  std::vector<RenderTexture *>
      m_renderTextures; //!< An array with all layer textures
  FrameBuffer *m_mainTextureFrameBuffer =
      nullptr; //!< The FrameBuffer which is used to render on the main texture
  std::map<Texture *, FrameBuffer *>
      m_frameBuffersOfTileSets; //!< The map which stores the FrameBuffers of
                                //!< the tileset textures
  Texture *m_clearTexture =
      nullptr; //!< The texture which is used for clearing some tiles
  bool m_renderObjects = false; //!< Defines wether to render the objects

  std::vector<Texture *>
      m_tilesetTextures; //!< An array which stores all tileset textures
};
} // namespace Johnny
