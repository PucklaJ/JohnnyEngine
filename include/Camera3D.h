...
public:
  ...
  /*! \return A 4x4 matrix to transform a 3D object from world space to view
   * space
   *
   */
   ...
   */
  void setLookDirection(GLfloat x, GLfloat y, GLfloat z) {
    m_lookDirection = Vector3f(x, y, z);
  }
  /*! \brief Sets the rotation of the camera
   *
   *
   * It is overridong the method from TransformableObject3D
   * \param v The vector which contains the x,y and z with which to rotate the
   * camera (in degrees)
   */
   ...
   * \param x The value with which to rotate the camera around the x-Axis (in
   * degrees) \param y The value with which to rotate the camera around the
   * y-Axis (in degrees) \param z The value with which to rotate the camera
   ...
   */
  void addPosition(GLfloat x, GLfloat y, GLfloat z) override {
    addPosition(Vector3f(x, y, z));
  }
  /*! \brief Adds a vector to the position of the camera
   * \param pos The vector to add to the position of the camera
   * \param relative Defines wether the addition should be relative to the
   * rotation
   */
