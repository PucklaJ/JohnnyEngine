...
public:
   ...
   *  \param full if true the Window goes into fullscreen or stays in
   * fullscreen, if false the Window goes into windowed mode or stays in
   * windowed mode \param flags The flags for the fullscreen transition
   */
  void setFullscreen(bool full = true, bool desktop = true);
  /*! \brief Sets the window into borderless mode
   *  \param border if true the window switches into borderless mode or stays in
   * borderless mode
   */
   ...
   */
  void setIcon(Texture *tex, GLenum target = GL_TEXTURE_2D,
...
private:
  MainClass *m_mainClass =
      nullptr;    //!< The MainClass which was passed through the constructor
