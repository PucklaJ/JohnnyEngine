...
public:
   ...
   *  \return The Model3D with the given name and nullptr if no Model3D was
  ...
  std::vector<DirectionalLight3D *>
      directionalLights; //!< All loaded DirectionLight3Ds (unused)
  std::vector<PointLight3D *>
      pointLights; //!< All loaded PointLight3Ds (unused)
};
...
public:
   ...
   */
  Texture *toTexture(GLenum target = GL_TEXTURE_2D,
...
/*! \brief A class which handles all resources such as Textures, Models, Shaders
 * etc.
 *
 */
...
public:
   ...
   *  \param file The file path of the source file from which to load the
   * contents relative to res/shaders \param defineChanges A map which contains
   * defines and their values which will be changed in the shader source file
   * (if nullptr no defines will be changed) \return The contents of the shader
   * source file or "" if the file wasn't found
   */
  std::string
  loadShader(const std::string &file,
             std::map<std::string, std::string> *defineChanges = nullptr);
...
private:
   ...
   *  \param switchYandZ Defines if the y and z coordinate of the vertex
  ...
  std::map<std::string, Texture *>
      m_textures; //!< The map that stores all Textures
  std::map<std::string, TextureData *>
      m_textureBytes; //!< The map that stores all TextureDatas
  std::map<std::string, std::string>
      m_shaders; //!< The map that stores all shader source file contents
  ...
  std::map<std::pair<std::string, unsigned short>, TTF_Font *>
      m_fonts; //!< The map which stores all fonts with the corresponding
               //!< filepath and point size
};
} // namespace Johnny
