...
namespace Johnny {
/*! \brief This class holds the values r,g,b and a which are used for all
 * coloring actions \param T The type of the r,g,b and a values \param maxValue
 * The max value of the r,g,b and a values
 */
template <class T, size_t maxValue> class Color : public Vector4<T> {
public:
...
inline Color<T, maxValue>::Color(const T &_r, const T &_g, const T &_b,
                                 const T &_a)
    : Vector4<T>(clamp(_r, (T)0, (T)maxValue), clamp(_g, (T)0, (T)maxValue),
                 clamp(_b, (T)0, (T)maxValue), clamp(_a, (T)0, (T)maxValue)) {}

template <class T, size_t maxValue>
inline Color<T, maxValue>::Color(const T &_r, const T &_g, const T &_b)
    : Color<T, maxValue>(_r, _g, _b, (T)maxValue) {}
...
template <class T, size_t maxValue>
template <class T1, size_t maxValue1>
inline Color<T, maxValue> &
Color<T, maxValue>::operator=(const Color<T1, maxValue1> &color) {
...
inline std::ostream &operator<<(std::ostream &os,
                                const Color<GLubyte, 255> &color) {
  os << "(" << (GLuint)color.r << ";" << (GLuint)color.g << ";"
     << (GLuint)color.b << ";" << (GLuint)color.a << ")";
  return os;
}
