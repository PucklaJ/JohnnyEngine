...
#define ERROR_OUT(func, errorText)                                             \
  if (func < 0)                                                                \
  Johnny::LogManager::error(                                                   \
      std::string(errorText) +                                                 \
      MainClass::getInstance()->getFramework()->getError())
...
enum FlagsInitFramework : FrameworkInitFlags {
  ...
  NoParachute = 1 << 7,
  Everything = Measure_Time | Audio | Video | Joystick | Haptic |
               GameController | Events | NoParachute
};

enum class Frameworks { SDL, GLFW };
...
public:
   ...
   *  \param flags The WindowFlags for the Window which define different things
   * like the resizability
   */
  virtual Window *createWindow(const std::string &title, unsigned int x,
  ...
  virtual bool setWindowFullscreen(Window *window, bool fullscreen = true,
                                   bool desktop = true) = 0;
  virtual bool setWindowBorderless(Window *window, bool borderless = true) = 0;
  virtual bool setWindowIcon(Window *window, Texture *tex,
                             GLenum target = GL_TEXTURE_2D,
                             GLenum format = GL_RGBA,
                             GLenum type = GL_UNSIGNED_BYTE) = 0;
