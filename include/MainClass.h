...
enum InitFlags {
  ...
  JOYSTICK =
      1 << 2, //!< Initialises the JoystickManager to capture controller inputs
...
public:
   ...
   *  \param initFlags Ore'd together InitFlags to determine what will be
   * initialised \param title The title of the Window which will be created
   *  \param width The width of the Window in pixels
   *  \param height The height of the Window in pixels
   *  \param windowFlags The flags which will be used for the Window
   * initialisation (this value will be ore'd together with SDL_WINDOW_SHOWN |
   * SDL_WINDOW_OPENGL)
   */
  MainClass(unsigned int initFlags = 0, const std::string &title = "New Window",
            unsigned int width = NORM_W, unsigned int height = NORM_H,
  ...
  /*! \return The back buffer of the object which is the texture to which
   * everything will be drawn and which will then be drawn to the screen
   *
   */
  FrameBuffer *getBackBuffer() { return m_frameBufferMulti; }
  /*! \return The back buffer mesh of the object which is the mesh which will be
   * drawn to the screen with the back buffer on it
   *
   */
  ...
  /*! \return The Physics2D of the object which handles all 2D physics related
   * things
   *
   */
  ...
  /*! \return The background color of the object which will be used to clear the
   * screen
   *
   */
  ...
  /*! \brief Gets called when the Window switches to fullscreen or switches to
   * windowed mode \param full true if the window switches to fullscreen
   * otherwise false
   */
...
protected:
  ...
  ResourceManager *m_resourceManager =
      nullptr;                    //!< The ResourceManager of the object
  ...
  JoystickManager *m_joystickManager =
      nullptr;                        //!< The JoystickManager of the object
...
private:
  ...
  /*! \brief Gets called when m_initFlags contains InitFlags::INIT_3D and
   * initialises all 3D rendering related things
   *
   */
  void init3D();
  /*! \brief Gets called when m_initFlags contains InitFlags::INIT_2D and
   * initialises all 2D rendering related things
   *
   */
  void init2D();
  /*! \brief Gets called when m_initFlags contains InitFlags::JOYSTICK and
   * initialises the JoystickManager
   *
   */
  ...
  Mesh3D *m_frameBufferMesh = nullptr; //!< The Mesh which will be drawn on the
                                       //!< screen with the back buffer on it
  FrameBuffer *m_frameBufferMulti =
      nullptr; //!< The FrameBuffer which is used to draw to the back buffer
  FrameBuffer *m_frameBuffer =
      nullptr; //!< The FrameBuffer which will be used to draw to the screen
  RenderBuffer *m_renderBufferMulti =
      nullptr; //!< The RenderBuffer which will be attached to
               //!< m_frameBufferMulti for depth rendering
  RenderBuffer *m_renderBuffer =
      nullptr; //!< The RenderBuffer which will be attached to m_frameBuffer for
               //!< depth rendering
  Texture *m_frameBufferTexMulti =
      nullptr; //!< The Texture which will be attached to m_frameBufferMulti for
               //!< color rendering
  Texture *m_frameBufferTex = nullptr; //!< The Texture which will be attached
                                       //!< to m_frameBuffer for color rendering

  std::string m_windowTitle;   //!< The title of the Window which was passed
                               //!< through the constructor
  unsigned int m_windowWidth;  //!< The width of the Window which was passed
                               //!< through the constructor in pixels
  unsigned int m_windowHeight; //!< The height of the Window which was passed
                               //!< through the constructor in pixels

  float m_scaleW; //!< Native_Resolution.x / Window_Resolution.x
  float m_scaleH; //!< Native_Resolution.y / Window_Resolution.y

  GLint m_viewportOffsetX =
      0; //!< The x offset of the back buffer on the screen
  GLint m_viewportOffsetY =
      0; //!< The y offset of the back buffer on the screen

  Colorb m_backgroundColor; //!< The bakground color which will be used to clear
                            //!< the screen

  Vector2f m_nativeResolution; //!< The resolution of the back buffer
  WindowFlags m_initWindowFlags =
      0; //!< The windowFlags which were passed through the constructor
  unsigned int m_initFlags =
      0; //!< The initFlags which were passed through the constructor
};
} // namespace Johnny
