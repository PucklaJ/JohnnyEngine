...
/*! \brief A class which handles the rendering of the scene, the shaders and
 * rendering of shadow maps
 *
 */
class RenderManager {
public:
  static Shader *DEFAULT_SHADER; //!< The default Shader which renders a 3D
                                 //!< model with materials
  static Shader *DEFAULT_SHADOWMAP_SHADER; //!< The default Shader for rendering
                                           //!< for a ShadowMap3D
  static Shader
      *DEFAULT_POST_PROCESSING_SHADER; //!< The default Shader for rendering the
  ...
  /*! \brief Adds a Shader so that it will be used for rendering with the
   * RenderManager \param s The Shader to add
   */
  ...
  /*! \brief Renders every Actor rwith its Shader that has been added to the
   *RenderManager \param m The MainClass from which to pick variables
   */
...
private:
  std::map<Shader *, std::vector<Actor *>>
      m_shaderActors; //!< All added Shaders and Actors
};
} // namespace Johnny
