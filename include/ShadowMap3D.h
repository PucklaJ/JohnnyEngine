...
namespace Johnny
{
    ...
    /*! \brief A class which represents a shadow map which is used to store the depth of every object which casts shadows
     *
     */
	...
	public:
		...
		/*! \brief Sets light which casts the shadows on the map to a DirectionalLight3D
		 *  \param d The DirectionalLight3D to set
		 */
		...
		/*! \brief Loads the light projection matrix and the Texture into a Shader as a uniform 
		 *  \param s The Shader to load to
		 *  \param index The index x to which the Texture should be bound
		 *  \param matrixName The name of the uniform for the light projection matrix
		 *  \param textureName The name of the uniform for the Texture
		 */
		void load(Shader* s, GLuint index = 0, const std::string& matrixName = "lightSpaceMatrix", const std::string& textureName = "shadowMap");
	...
	private:
		...
		FrameBuffer* m_frameBuffer = nullptr;			  //!< The FrameBuffer which is used for rendering to the Texture
		PointLight3D* m_pointLight = nullptr;			  //!< The PointLight3D which casts the shadows
		DirectionalLight3D* m_directionalLight = nullptr; //!< The DirectionalLight3D which casts the shadows
		SpotLight3D* m_spotLight = nullptr;				  //!< The SpotLight3D which casts the shadows
		GLuint m_curLight = 0;							  //!< The type of the light which casts the shadows

		Matrix4f m_lightSpaceMatrix;					  //!< The light space projection matrix which is used to transform the vertices into the view of the light
	};
