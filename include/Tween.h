...
public:
   ...
   *  \return true if the Tween finished and should be deleted and false
   * otherwhise
   */
...
protected:
  ...
  TweenableObject2D *m_parent =
      nullptr; //!< The parent TweenableObject2D which will be animated
...
private:
  static std::vector<int>
      m_ids; //!< An array which stores all ids of every Tween
};
...
public:
   ...
   */
  AnimationTween2D(Texture *atlas, int x, int y, float time, bool loop = true,
   ...
   */
  AnimationTween2D(const std::vector<Texture *> &textures, float time,
                   bool loop = true, bool customTime = false);
  /*! \brief Creates a new AnimationTween2D
   *  \param regions The TextureRegions which represent the region of each
   * animation frame on the texture \param time The time of one animation frame
   * in sec \param loop Wether the animation should loop \param customTime
   * Wether the time should be added manually
   */
  AnimationTween2D(const std::vector<TextureRegion> &regions, float time,
                   bool loop = true, bool customTime = false);
  ~AnimationTween2D();
...
private:
  ...
  std::vector<Texture *> *m_textures =
      nullptr; //!< The vector which stores all animation frames if the
               //!< corresponding constructor was used
  std::vector<TextureRegion> *m_regions =
      nullptr; //!< The vector which stores all TextureRegions of all animation
               //!< frames if the corresponding construct was used
  Texture *m_atlas =
      nullptr; //!< The texture atlas from it should take the animation frames
               //!< if the corresponding constructor was used
  ...
  TextureRegion m_srcRegion = {
      0, 0, 0, 0};    //!< The region which defines what to take from the atlas
...
public:
   ...
   *  \return The Tween2D with the given id or nullptr if no Tween2D with te id
   * was found
   */
...
private:
  std::vector<Tween2D *>
      m_tweens; //!< The vector which stores all Tween2Dsof the object
};
} // namespace Johnny
