...
namespace Johnny {
void ShadowMap3DShaderUpdater::setUniforms(Transform3D *m,
...
void ShadowMap3DShaderUpdater::setUniforms(Entity3D *m,
                                           const unsigned int index) {
  m->getModel()->render(m_shader);
}

ShadowMap3D::ShadowMap3D(GLsizei width, GLsizei height)
    : Texture(nullptr, width, height, GL_LINEAR, GL_DEPTH_COMPONENT,
...
ShadowMap3D::~ShadowMap3D() { delete m_frameBuffer; }
...
void ShadowMap3D::load(Shader *s, GLuint index, const std::string &matrixName,
...
void ShadowMap3D::configureMatricesAndShader(Shader *s) {
  ...
  if (m_curLight == SHADOW_DIR) {
    ...
    lightView =
        Matrix4f::lookAt(-direction * 1.3f, -direction * 1.3f + direction,
                         Vector3f(0.0f, 1.0f, 0.0f));
    lightProjection =
        Matrix4f::orthographic(100.0f, -100.0f, 100.0f, -100.0f, 100.0f, 0.1f);

  } else if (m_curLight == SHADOW_SPOT) {
    direction = m_spotLight->direction;
    position = m_spotLight->position;

    lightView = Matrix4f::lookAt(position, position + direction,
                                 Vector3f(0.0f, 1.0f, 0.0f));
    lightProjection = Matrix4f::perspective(
        70.0f, (float)m_width / (float)m_height,
        clamp(1000.0f, 100.0f, 1000.0f), clamp(0.1f, 1.0f, 10.0f));
  }
