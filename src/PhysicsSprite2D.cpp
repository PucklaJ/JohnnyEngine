...
namespace Johnny {
PhysicsSprite2D::PhysicsSprite2D(b2Body *body) : Sprite2D(), m_body(body) {
  setName("PhysicsSprite2D");
}
PhysicsSprite2D::PhysicsSprite2D(const std::string &file, b2Body *body)
    : Sprite2D(file), m_body(body) {
  setName("PhysicsSprite2D");
}
PhysicsSprite2D::PhysicsSprite2D(Texture *tex, b2Body *body)
    : Sprite2D(tex), m_body(body) {
  setName("PhysicsSprite2D");
}
...
bool PhysicsSprite2D::init() {
  if (!m_mainClass->getPhysics2D())
    LogManager::error("Couldn't initialise PhysicsSprite2D! You need to add "
                      "PHYSICS_2D to the InitFlags!");
...
void PhysicsSprite2D::attachBodyToSprite() {
  setPosition(
      m_mainClass->getPhysics2D()->coordsWorldToPixel(m_body->GetPosition()) -
      (getDrawSize() / 2.0f) + m_offset);
