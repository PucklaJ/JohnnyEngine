...
namespace Johnny {
inline int calculateF(const A_Square &pos, const A_Square &target,
                      int previousCosts) {
  return (int)(abs(target.x - pos.x) + abs(target.y - pos.y) + pos.g +
               previousCosts);
}
...
inline int getBestSquareIndex(const std::vector<A_Square *> &openList,
                              const A_Square &target, int &previousCosts) {
...
// up right down left
inline void getNeighbourIndices(const A_Square &pos,
                                const std::vector<A_Square> &squares, int w,
                                int indices[]) {
  indices[0] = -1;
  indices[1] = -1;
  indices[2] = -1;
...
inline void removeFromOpen(std::vector<A_Square *> &openList,
                           const A_Square &s) {
...
std::vector<A_Square> a_stern(std::vector<A_Square> &field, A_Square &start,
                              A_Square &target, int w) {
  ...
  do {
    ...
    if (!squareEquals(*current, target)) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (!neighbourSquare->solid &&
            !Contains(closedList, *neighbourSquare) &&
            !Contains(openList, *neighbourSquare)) {
