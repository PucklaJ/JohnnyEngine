...
void JoystickManager::addController(void *j) {
  for (size_t i = 0; i < m_controllers.size(); i++) {
    if (m_controllers[i] == nullptr)
      continue;
    if (MainClass::getInstance()->getFramework()->isSameController(
            j, m_controllers[i]))
      return;
  }
...
void JoystickManager::removeController(void *j) {
  for (size_t i = 0; i < m_controllers.size(); i++) {
    if (MainClass::getInstance()->getFramework()->isSameController(
            j, m_controllers[i])) {
      MainClass::getInstance()->getFramework()->closeController(
          m_controllers[i]);
...
void JoystickManager::update() {
  ...
  for (size_t i = 0; i < m_controllers.size(); i++) {
    ...
    if (MainClass::getInstance()->getFramework()->isAttachedController(
