...
Skybox::Skybox() : Actor("Skybox") { m_castsShadows3D = false; }
...
bool Skybox::init() {
  ...
  m_cubeMap = new CubeMap3D(
      m_textures[SkyboxTex::SKY_TOP], m_textures[SkyboxTex::SKY_BOTTOM],
      m_textures[SkyboxTex::SKY_LEFT], m_textures[SkyboxTex::SKY_RIGHT],
      m_textures[SkyboxTex::SKY_FRONT], m_textures[SkyboxTex::SKY_BACK],
      m_mainClass->getResourceManager());

  if (SKYBOX_SHADER == nullptr) {
    SKYBOX_SHADER = new Shader();
    SKYBOX_SHADER->addVertexShader(
        m_mainClass->getResourceManager()->loadShader(
            "vertexShaderSkybox.glsl"));
    SKYBOX_SHADER->addFragmentShader(
        m_mainClass->getResourceManager()->loadShader(
            "fragmentShaderSkybox.glsl"));
...
bool Skybox::update() { return true; }
...
void Skybox::quit() { delete m_cubeMap; }
...
SkyboxMesh::SkyboxMesh() {
  ...
  if (m_vbo == 0) {
    LogManager::error("Couldn't generate Vertex Buffer for SkyboxMesh", true,
                      true, false);
  } else {
    glGenVertexArrays(1, &m_vao);
    if (m_vao == 0) {
      LogManager::error("Couldn't generate VertexArray for SkyboxMesh", true,
                        true, false);
    }
  }
}
...
void SkyboxMesh::addVertices(SkyboxVertex *vertices, unsigned int numVertices) {
  ...
  glBufferData(GL_ARRAY_BUFFER, m_numVertices * sizeof(SkyboxVertex), vertices,
               GL_STATIC_DRAW);
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(SkyboxVertex),
                        (GLvoid *)offsetof(SkyboxVertex, position));
