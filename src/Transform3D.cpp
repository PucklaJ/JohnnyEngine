...
Matrix4f Transform3D::getProjectionMatrix() { return m_projectionMatrix; }
...
void Transform3D::setTranslation(const Vector3f &vec) { m_translation = vec; }
...
Matrix4f Transform3D::getTransformation() const {
  return Matrix4f::translate(m_translation) *
         (Matrix4f::rotate(m_rotation.y, Vector3f(0.0f, 1.0f, 0.0f)) *
...
void Transform3D::setRotation(const Vector3f &vec) { m_rotation = vec; }

void Transform3D::setRotation(GLfloat x, GLfloat y, GLfloat z) {
  setRotation(Vector3f(x, y, z));
}

void Transform3D::setScale(const Vector3f &vec) { m_scale = vec; }
...
Matrix4f
TransformableObject3D::getProjectedTransformation(const Camera3D *cam) const {
