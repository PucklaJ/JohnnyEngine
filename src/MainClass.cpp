...
MainClass::MainClass(unsigned int initFlags, const std::string &title,
                     unsigned int width, unsigned int height,
                     WindowFlags windowFlags)
    : Actor("MainClass"), m_windowTitle(title), m_windowWidth(width),
      m_windowHeight(height), m_scaleW(1.0f), m_scaleH(1.0f),
      m_initWindowFlags(windowFlags), m_initFlags(initFlags)
...
void MainClass::m_quit() {
  ...
  if (!m_framework->quit())
    LogManager::error(std::string("Error while quitting: ") +
                      m_framework->getError());
}
...
void MainClass::m_init() {
  ...
  if (!m_framework->init(FlagsInitFramework::Everything)) {
    LogManager::error(std::string("Initializing Framework: ") +
                      m_framework->getError());
  }
  if (TTF_Init() < 0) {
  ...
  m_window = m_framework->createWindow(
      m_windowTitle, 150, 150, m_windowWidth, m_windowHeight,
      FlagsWindow::Shown | FlagsWindow::OpenGL | m_initWindowFlags);

  if (!m_window || !m_window->getHandle()) {
    LogManager::error(std::string("Creating Window: ") +
                      m_framework->getError());
  }
  if (!m_framework->createOpenGLContext()) {
    LogManager::error(std::string("Couldn't create OpenGL-Context: ") +
                      m_framework->getError());
  }
  ...
  m_nativeResolution =
      Vector2f((GLfloat)m_windowWidth, (GLfloat)m_windowHeight);
  ...
  m_frameBufferTex = new Texture(
      nullptr, (GLsizei)m_nativeResolution.x, (GLsizei)m_nativeResolution.y,
      Settings::geti(SettingName::BACK_BUFFER_FILTERING), GL_RGB, GL_RGB,
      GL_UNSIGNED_BYTE, GL_TEXTURE_2D);
  m_frameBufferTexMulti = new Texture(
      nullptr, (GLsizei)m_nativeResolution.x, (GLsizei)m_nativeResolution.y,
      Settings::geti(SettingName::BACK_BUFFER_FILTERING), GL_RGB, GL_RGB,
      GL_UNSIGNED_BYTE, GL_TEXTURE_2D_MULTISAMPLE);
  m_renderBuffer = new RenderBuffer(
      (GLsizei)m_nativeResolution.x, (GLsizei)m_nativeResolution.y,
      GL_DEPTH24_STENCIL8, GL_RENDERBUFFER, false);
  m_renderBufferMulti = new RenderBuffer(
      (GLsizei)m_nativeResolution.x, (GLsizei)m_nativeResolution.y,
      GL_DEPTH24_STENCIL8, GL_RENDERBUFFER, true);
  m_frameBuffer->addTexture(m_frameBufferTex, GL_COLOR_ATTACHMENT0,
                            GL_FRAMEBUFFER, GL_TEXTURE_2D);
  m_frameBuffer->addRenderBuffer(m_renderBuffer, GL_DEPTH_STENCIL_ATTACHMENT,
                                 GL_FRAMEBUFFER);
  m_frameBufferMulti->addTexture(m_frameBufferTexMulti, GL_COLOR_ATTACHMENT0,
                                 GL_FRAMEBUFFER, GL_TEXTURE_2D_MULTISAMPLE);
  m_frameBufferMulti->addRenderBuffer(
      m_renderBufferMulti, GL_DEPTH_STENCIL_ATTACHMENT, GL_FRAMEBUFFER);
  ...
  Transform3D::setProjection(Matrix4f::perspective(
      70.0f, m_nativeResolution.x / m_nativeResolution.y, 1000.0f, 0.1f));
...
bool MainClass::init() { return true; }
...
void MainClass::mainLoop() {
  ...
  while (true) {
    ...
    if (m_lighting3D)
      m_lighting3D->renderShadowMaps(this,
                                     RenderManager::DEFAULT_SHADOWMAP_SHADER);
    render();
    m_renderManager->render(this);

    m_frameBufferMulti->blit(m_frameBuffer, 0, 0, (GLint)m_nativeResolution.x,
                             (GLint)m_nativeResolution.y, 0, 0,
                             (GLint)m_nativeResolution.x,
                             (GLint)m_nativeResolution.y);
    m_frameBufferMulti->unbind();

    RenderUtil::clearScreen();
    glDisable(GL_DEPTH_TEST);
    glViewport(m_viewportOffsetX, m_viewportOffsetY, m_windowWidth,
               m_windowHeight);
    RenderManager::DEFAULT_POST_PROCESSING_SHADER->bind();
    m_frameBufferTex->bind(RenderManager::DEFAULT_POST_PROCESSING_SHADER,
                           "frameBuffer");

    m_frameBufferMesh->render();

    m_frameBufferTex->unbind();
    glViewport(0, 0, (GLsizei)m_nativeResolution.x,
               (GLsizei)m_nativeResolution.y);
...
bool MainClass::pollEvents() {
  ...
  while (m_framework->pollEvent()) {
    ...
    case EventType::MouseMotion:
      m_inputManager->setMouseCoords(m_framework->event.motion.x,
                                     m_framework->event.motion.y);
      m_inputManager->setMouseRel(m_framework->event.motion.xrel,
                                  m_framework->event.motion.yrel);
      break;
    case EventType::MouseWheel:
      m_inputManager->setMouseWheel(m_framework->event.wheel.x,
                                    m_framework->event.wheel.y);
      break;
    case EventType::WindowEvent:
      switch (m_framework->event.window.event) {
      case WindowEventType::SizeChanged:
        onResize(m_framework->event.window.data1,
                 m_framework->event.window.data2);
        break;
      default:
        break;
      }
...
bool MainClass::pollEvent(const Event &e) { return true; }

bool MainClass::render() { return true; }
...
bool MainClass::update() { return true; }

Camera3D *MainClass::getCamera3D() { return m_camera3D; }

Camera2D *MainClass::getCamera2D() { return m_camera2D; }
...
void MainClass::onFullscreen(bool full) {
  ...
  if (((normScale / newScale) > 0.999 && (normScale / newScale) < 1.001)) {
    ...
    if (newRes.y > (int)m_windowHeight)
      m_viewportOffsetY =
          (GLint)(((float)newRes.y - (float)m_windowHeight) / 2.0f);
    else {
      m_windowWidth = (int)((double)newRes.y * normScale);
      m_windowHeight = newRes.y;
      m_viewportOffsetX =
          (GLint)(((float)newRes.x - (float)m_windowWidth) / 2.0f);
    }
  }
}
...
void MainClass::setNativeRes(const Vector2f &v) {
  ...
  Transform3D::setProjection(Matrix4f::perspective(
      70.0f, getNativeRes().x / getNativeRes().y, 1000.0f, 0.1f));
...
void MainClass::renderSceneForShadowMap(Shader *s) {
  ...
  for (size_t i = 0; i < actors.size(); i++) {
    if (actors[i]->getMainClass() && actors[i]->castsShadows() &&
        actors[i]->isVisible()) {
