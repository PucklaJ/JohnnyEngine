...
Sprite2D::Sprite2D(const std::string &fileName)
    : Actor("Sprite2D"), m_fileName(fileName) {
  m_castsShadows3D = false;
  m_drawScale = Vector2f(1.0f, 1.0f);
}
...
bool Sprite2D::init() {
  if (!Texture::getTexture2DShader())
    LogManager::error("Couldn't initialise Sprite2D! You have to add INIT_2D "
                      "to the InitFlags!");
  ...
  if (!m_texture && m_fileName != "") {
    setTexture(m_mainClass->getResourceManager()->loadTexture(
        m_fileName, Settings::geti(SettingName::SPRITE2D_FILTERING)));
  }
...
void Sprite2D::setTexture(Texture *texture) {
  ...
  if (texture) {
    ...
    setSrcRegion(
        TextureRegion(0, 0, texture->getWidth(), texture->getHeight()));
  }
}

void Sprite2D::setSrcRegion(const TextureRegion &r) { m_srcRegion = r; }
void Sprite2D::setDrawSize(const Vector2f &vec) {
  m_drawScale = Vector2f(vec.x / (GLfloat)m_texture->getWidth(),
                         vec.y / (GLfloat)m_texture->getHeight());
}
...
Vector2f Sprite2D::getDrawSize() const {
  return Vector2f((GLfloat)m_texture->getWidth(),
                  (GLfloat)m_texture->getHeight()) *
         m_drawScale;
}
Vector2f Sprite2D::getActualSize() const { return getDrawSize() * getScale(); }
...
bool Sprite2D::intersects(Sprite2D *spr) {
  ...
  return pos.x < pos1.x + size1.width && pos.x + size.width > pos1.x &&
         pos.y < pos1.y + size1.height && pos.y + size.height > pos1.y;
}
...
bool Sprite2D::intersects(const Vector2f &pos) {
  ...
  return pos.x < pos1.x + size1.width && pos.x > pos1.x &&
         pos.y < pos1.y + size1.height && pos.y > pos1.y;
}
...
bool Sprite2D::intersects(const Rectangle<GLfloat> &rect) {
  ...
  return rect.x < pos1.x + size1.width && rect.x + rect.width > pos1.x &&
         rect.y < pos1.y + size1.height && rect.y + rect.height > pos1.y;
}
} // namespace Johnny
