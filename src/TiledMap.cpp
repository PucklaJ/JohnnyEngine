...
namespace Johnny
{
    TiledMap::TiledMap(const std::string& file) : Sprite2D(nullptr),Tmx::Map()
    {
        ...
        if(HasError())
        {
            LogManager::error(std::string("TiledMap Error (") + std::to_string(GetErrorCode()) + ") : " + GetErrorText());
            m_works = false;
        }
    }
    ...
    bool TiledMap::init()
    {
        ...
        if(m_works)
        {
            ...
            {
                LogManager::error(std::string("This Orientation is not supported (") + std::to_string(GetOrientation()) + ")");
                return true;
            }
    ...
    void TiledMap::loadTexture()
    {
        ...
        m_texture = new Texture(nullptr,GetWidth() * GetTileWidth(),GetHeight() * GetTileHeight(),(GLenum)Settings::geti(SettingName::SPRITE2D_FILTERING));
    ...
    void TiledMap::loadTilesets()
    {
        ...
        for(int i = 0;i<GetNumTilesets();i++)
        {
            ...
            fr->addTexture(tex);
            if(!fr->checkStatus())
                LogManager::error("FrameBuffer of Tileset \"" + imageFile + "\" is not completed!");
                
            m_frameBuffersOfTileSets.insert(std::pair<Texture*,FrameBuffer*>(tex,fr));
    ...
    void TiledMap::loadCollisionObjects(const Tmx::Tile* tile,int x,int y)
    {
        ...
        if(!m_mainClass->getPhysics2D())
            LogManager::error("The Physics2D aren't activated! You need to add PHYSICS2D to the InitFlags!");
    ...
    void TiledMap::loadCollisionPolygon(const Tmx::Object* obj,int x,int y)
    {
        ...
        {
        	LogManager::error(std::string("Polygon (") + std::to_string(x) + "; " + std::to_string(y) + ") has too much or too few vertices");
        	return;
        }
        ...
        Vector2f pixelPos((GLfloat)x+(GLfloat)GetTileWidth()/2.0f,(GLfloat)y+(GLfloat)GetTileHeight()/2.0f);
    ...
    void TiledMap::loadCollisionPolyline(const Tmx::Object* obj,int x,int y)
    {
		...
		{
			LogManager::log(std::string("Polyline (") + std::to_string(x) + "; " + std::to_string(y) + ") has too much or too few vertices");
			return;
		}
    ...
    void TiledMap::loadCollisionRectangle(const Tmx::Object* obj,int x,int y)
    {
        ...
        bdef.position = m_mainClass->getPhysics2D()->coordsPixelToWorld(Vector2f((GLfloat)x+(GLfloat)obj->GetX()+(GLfloat)obj->GetWidth()/2.0f,(GLfloat)y+(GLfloat)obj->GetY()+(GLfloat)obj->GetHeight()/2.0f));
		std::string type = props.GetStringProperty("Type","static");
		bdef.type = (type == "static" ? b2_staticBody : (type == "dynamic" ? b2_dynamicBody : b2_kinematicBody));
        ...
        shape.SetAsBox(m_mainClass->getPhysics2D()->scalarPixelToWorld((GLfloat)obj->GetWidth()/2.0f),m_mainClass->getPhysics2D()->scalarPixelToWorld((GLfloat)obj->GetHeight()/2.0f));
        fdef.shape = &shape;
    ...
    void TiledMap::getSrcRegionForTileID(TextureRegion* src,int numTilesX,int index,int id)
    {
        src->x = id % numTilesX;
        src->y = (id - src->x) / numTilesX;
    	...
    	for(int x = 0;x<layer->GetWidth();x++)
    	{
    		for(int y = 0;y<layer->GetHeight();y++)
    		{
    			...
    			if(m_animations.count(tile->GetId()) == 0)
    			{
					...
					Texture* texture = m_mainClass->getResourceManager()->loadTexture(GetFilepath() + tileset->GetImage()->GetSource());
					...
					for(size_t i = 0;i<anims.size();i++)
					{
						...
						getSrcRegionForTileID(&region,m_tilesX[index],index,anims[i].GetTileID());
    ...
    void TiledMap::renderAnimations()
    {
    	...
    	for(size_t i = 0;i<m_animTiles.size();i++)
    	{
            ...
            Texture::renderTexture2D(anim.texture,&dstRegion,&anim.regions[anim.curRegion]);
        }
...
//		void blit(FrameBuffer*, GLint, GLint, GLint, GLint, GLint, GLint, GLint, GLint, GLbitfield mask = GL_COLOR_BUFFER_BIT, GLenum filter = GL_NEAREST);
    	...
    	{
           m_renderTextures[i]->render(Vector2f(0.0f,0.0f),Vector2f(1.0f,1.0f),0.0f,nullptr,nullptr,true,GL_TEXTURE_2D);
    	}
    ...
    void TiledMap::quit()
    {
        ...
        for(std::map<Texture*,FrameBuffer*>::iterator it = m_frameBuffersOfTileSets.begin();it!=m_frameBuffersOfTileSets.end();it++)
        {
            delete it->second;
        }
    ...
    void TiledMap::destroyTile(int x,int y,int layer)
    {
    	...
    	//TextureRegion region = {x*GetTileWidth(),y*GetTileHeight(),GetTileWidth(),GetTileHeight()};
    ...
    void TiledMap::setTile(int x,int y,int layer,int tileset,int id)
    {
    	...
    	if(tile && tile->GetFrameCount() > 0 && !animLoaded)
    	{
    		...
    		for(size_t i = 0;i<tile->GetFrames().size();i++)
    		{
    			...
    			getSrcRegionForTileID(&srcRegion,m_tilesX[tileset],tileset,frame.GetTileID());
