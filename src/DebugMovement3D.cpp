...
namespace Johnny {
DebugMovement3D::DebugMovement3D(float moveSpeed, float lookSpeed)
    : Actor("DebugMovement"), m_moveSpeed(moveSpeed), m_lookSpeed(lookSpeed) {
  m_castsShadows3D = false;
}

DebugMovement3D::~DebugMovement3D() {}

bool DebugMovement3D::init() { return true; }
...
bool DebugMovement3D::update() {
  ...
  if (m_mainClass->getInputManager()->isPressed(Keys::W)) {
    m_mainClass->getCamera3D()->addPosition(
        0.0, 0.0,
        -m_moveSpeed * m_mainClass->getTimer()->getDeltaTimeInSeconds(), true);
  }
  ...
  if (m_mainClass->getInputManager()->isPressed(Keys::A)) {
    m_mainClass->getCamera3D()->addPosition(
        -m_moveSpeed * m_mainClass->getTimer()->getDeltaTimeInSeconds(), 0.0,
        0.0, true);
  }
  ...
  if (m_mainClass->getInputManager()->isPressed(Keys::K)) {
    m_mainClass->getCamera3D()->addPosition(
        0.0, -m_moveSpeed * m_mainClass->getTimer()->getDeltaTimeInSeconds(),
        0.0, false);
  }
  ...
  if (m_mainClass->getFramework()->isCursorHidden()) {
    GLfloat x =
        -m_lookSpeed * (float)m_mainClass->getInputManager()->getMouse().yrel;
    GLfloat y =
        -m_lookSpeed * (float)m_mainClass->getInputManager()->getMouse().xrel;
    GLfloat z = m_lookSpeed *
                (float)m_mainClass->getInputManager()->getMouse().wheel_y *
                10.0f * m_mainClass->getTimer()->getDeltaTimeInSeconds();
