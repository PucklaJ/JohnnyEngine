...
Window::Window(void *handle)
    : m_mainClass(MainClass::getInstance()), m_handle(handle) {
  m_res = getResolution();
}
...
void Window::setResolution(int w, int h) { setResolution(Vector2i(w, h)); }

void Window::setPosition(const Vector2i &p) {
  m_mainClass->getFramework()->setWindowPosition(this, p);
}

void Window::setPosition(int x, int y) { setPosition(Vector2i(x, y)); }
...
bool Window::swap() { return m_mainClass->getFramework()->swapWindow(this); }
} // namespace Johnny
