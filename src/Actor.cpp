...
namespace Johnny {
template <class t> const char *toString(t number) {
  std::string str;
...
Actor::Actor(const char *name) : m_name(name), m_parent(nullptr) {
  do {
    m_id = rand();
...
#ifdef DEBUG_OUTPUTS
    std::cout << "Children[" << i << "]: " << m_children[i]->getName()
              << std::endl;
#endif
...
void Actor::setParent(Actor *a) { m_parent = a; }
...
void Actor::setMainClass(MainClass *mc) { m_mainClass = mc; }

MainClass *Actor::getMainClass() { return m_mainClass; }

bool Actor::init() { return true; }

void Actor::setAffectedByCamera(const bool b) { m_isAffectedByCamera = b; }

void Actor::setVisible(const bool b) { m_visible = b; }
...
bool Actor::isOnScreen() { return true; }
