...
Tween2D::Tween2D(float time) : m_time(time), m_parent(nullptr) {
  do {
    m_id = rand();
...
void Tween2D::setParent(TweenableObject2D *a) { m_parent = a; }

void Tween2D::addTime(float time) { m_passedTime += time; }

PositionTween2D::PositionTween2D(const Vector2f &v, float time)
    : Tween2D(time), m_dstPos(v) {}
...
void PositionTween2D::init() {
  m_vel.x = ((m_dstPos.x - m_parent->getPosition().x == 0.0f
                  ? 0.0f
                  : ((m_dstPos.x - m_parent->getPosition().x) / m_time)));
  m_vel.y = ((m_dstPos.y - m_parent->getPosition().y == 0.0f
                  ? 0.0f
                  : ((m_dstPos.y - m_parent->getPosition().y) / m_time)));
...
AnimationTween2D::AnimationTween2D(Texture *atlas, int x, int y, float time,
                                   bool loop, bool customTime)
    : Tween2D(time), m_sprParent(nullptr), m_atlas(atlas), m_x(x), m_y(y),
      m_loop(loop), m_customTime(customTime)

{}

AnimationTween2D::AnimationTween2D(const std::vector<Texture *> &texs,
                                   float time, bool loop, bool customTime)
    : Tween2D(time), m_sprParent(nullptr), m_x(0), m_y(0), m_loop(loop),
...
AnimationTween2D::AnimationTween2D(const std::vector<TextureRegion> &regions,
                                   float time, bool loop, bool customTime)
    : Tween2D(time), m_sprParent(nullptr), m_x(0), m_y(0), m_loop(loop),
      m_customTime(customTime) {
...
AnimationTween2D::~AnimationTween2D() {
  ...
  if (m_textures) {
    for (size_t i = 0; i < m_textures->size(); i++) {
      if (!MainClass::getInstance()->getResourceManager()->isLoaded(
              (*m_textures)[i])) {
        delete (*m_textures)[i];
      }
    }
...
void AnimationTween2D::init() {
  ...
  if (m_atlas) {
    ...
    m_sprParent->setDrawSize((GLfloat)m_srcRegion.width,
                             (GLfloat)m_srcRegion.height);
  } else if (m_textures) {
    m_srcRegion.x = 0;
    if (m_textures->empty()) {
...
PulsateTween2D::PulsateTween2D(float length, float speed, bool loop)
    : Tween2D(0.0), m_loop(loop), m_length(length), m_speed(speed) {}
...
RotationTween2D::RotationTween2D(float speed) : Tween2D(0.0f), m_speed(speed) {}
