...
void UniformBuffer::createBuffer(GLuint bindingPoint) {
  ...
  glGetError();
  glNamedBufferStorage(m_buffer, size, newData,
  ...
  error = glGetError();
  errorStr = std::string("Failed to bind UniformBuffer to BindingPoint ") +
             std::to_string(bindingPoint) + ": ";
  ...
  case GL_INVALID_VALUE:
    ...
    LogManager::error(errorStr + "BindingPoint is too big (MAX: " +
                      std::to_string(maxBindings - 1) + ")");
    break;
  default:
    LogManager::error(errorStr + std::to_string(error));
    break;
...
void UniformBuffer::addVariable(UBOTypes type) { m_types.push_back(type); }
...
void UniformBuffer::setVariable(UBOTypes type, unsigned int index,
                                GLvoid *data) {
  if (m_buffer == 0)
    return;
...
void UniformBuffer::m_setVariable(UBOTypes type, GLsizei offset, GLvoid *data,
  ...
  case ARRAY_MAT2:
    for (unsigned int i = 0; i < arraySize; i++) {
      ((GLfloat *)(m_data + offset + i * 2 * 16))[0] =
          ((GLfloat *)data)[0 + i * 4];
      ((GLfloat *)(m_data + offset + i * 2 * 16))[1] =
          ((GLfloat *)data)[1 + i * 4];
      ((GLfloat *)(m_data + offset + 16 + i * 2 * 16))[0] =
          ((GLfloat *)data)[2 + i * 4];
      ((GLfloat *)(m_data + offset + 16 + i * 2 * 16))[1] =
          ((GLfloat *)data)[3 + i * 4];
    }
    break;
  case ARRAY_MAT3:
    for (unsigned int a = 0; a < arraySize; a++) {
      for (unsigned int i = 0; i < 3; i++) {
        for (unsigned int j = 0; j < 3; j++) {
          *((GLfloat *)(m_data + addOffset)) =
              ((GLfloat *)data)[i * 3 + j + a * 3 * 3];
          addOffset += sizeof(GLfloat);
        }
        addOffset += sizeof(GLfloat);
      }
...
void Shader::link() {
  if (m_program == 0) {
    LogManager::error(
        "Couldn't link Shader, because the program hasn't been initialized");
    return;
  }
...
void Shader::attachUniformBuffer(const std::string &blockName,
                                 GLuint bindingPoint) {
  if (m_program == 0)
    return;
  ...
  GLenum error = glGetError();
  std::string errorStr = "Couldn't bind UniformBlock to BindingPoint " +
                         std::to_string(bindingPoint) + ": ";
  GLint maxBinding = 0;
  ...
  case GL_INVALID_VALUE:
    ...
    LogManager::error(errorStr +
                      (index == 0 ? "UniformBlock is invalid"
                                  : ("BindingPoint is too big (MAX: " +
                                     std::to_string(maxBinding - 1) + ")")));
    break;
  default:
    LogManager::error(errorStr + std::to_string(error));
    break;
...
void Shader::addProgram(const std::string &text, GLuint type) {
  if (m_program == 0) {
    LogManager::error("Couldn't add Shader to program, because the program "
