...
Shader::Shader(const std::string& name) : m_name(name)
{
	...
	if (m_program == 0)
	{
		SDL::LogManager::error("Couldn't create ShaderProgram");
	}
}
...
void Shader::link()
{
	...
	if (isLinked == GL_FALSE)
	{
		...
		SDL::LogManager::error(std::string("Shader ("+ m_name +") failed to link:\n") + infoLog);
	...
	if (status == GL_FALSE)
	{
		...
		SDL::LogManager::error(std::string("Shader ("+ m_name +") failed to validate:\n") + infoLog);
...
void Shader::addProgram(const std::string & text, GLuint type)
{
	if (m_program == 0)
	{
		SDL::LogManager::error("Couldn't add Shader to program, because the program hasn't been initialized");
		return;
	}
	...
	case GL_VERTEX_SHADER:
		...
		if (m_vert == 0)
		{
			SDL::LogManager::error("Couldn't create VertexShader");
		}
		break;
	case GL_FRAGMENT_SHADER:
		...
		if (m_frag == 0)
		{
			SDL::LogManager::error("Couldn't create FragmentShader");
		}
		break;
	case GL_GEOMETRY_SHADER:
		...
		if (m_geo == 0)
		{
			SDL::LogManager::error("Couldn't create GeometryShader");
		}
		break;
	}
