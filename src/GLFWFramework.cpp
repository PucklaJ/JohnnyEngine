...
bool GLFWFramework::createOpenGLContext() {
  glfwMakeContextCurrent(
      (GLFWwindow *)MainClass::getInstance()->getWindow()->getHandle());
  return true;
}

Window *GLFWFramework::createWindow(const std::string &title, unsigned int x,
                                    unsigned int y, unsigned int w,
                                    unsigned int h, WindowFlags flags) {
  Window *window = nullptr;
  GLFWwindow *handle = nullptr;

  glfwWindowHint(GLFW_RESIZABLE,
                 flags & FlagsWindow::Resizeable ? GLFW_TRUE : GLFW_FALSE);
  glfwWindowHint(GLFW_VISIBLE,
                 flags & FlagsWindow::Shown ? GLFW_TRUE : GLFW_FALSE);
  glfwWindowHint(GLFW_DECORATED,
                 flags & FlagsWindow::Borderless ? GLFW_FALSE : GLFW_TRUE);
  glfwWindowHint(GLFW_FOCUSED,
                 flags & FlagsWindow::Input_Focus ? GLFW_TRUE : GLFW_FALSE);

  handle = glfwCreateWindow(
      w, h, title.c_str(),
      (flags & FlagsWindow::Fullscreen) ? glfwGetPrimaryMonitor() : nullptr,
      nullptr);
  if (handle == nullptr)
    return nullptr;
...
std::string GLFWFramework::getError() { return errorString; }
bool GLFWFramework::pollEvent() {
  ...
  glfwGetCursorPos(
      (GLFWwindow *)MainClass::getInstance()->getWindow()->getHandle(),
      &m_prevMousePos.x, &m_prevMousePos.y);
...
bool GLFWFramework::setWindowFullscreen(Window *window, bool fullscreen,
                                        bool desktop) {
  Vector2i monitorRes;
  Vector2i pos;
  ...
  glfwSetWindowMonitor(
      ...
      fullscreen ? glfwGetPrimaryMonitor() : nullptr, fullscreen ? 0 : 150,
      fullscreen ? 0 : 150, fullscreen ? mode->width : res.width,
      fullscreen ? mode->height : res.height, mode->refreshRate);
  return true;
}
bool GLFWFramework::setWindowBorderless(Window *window, bool borderless) {
  return false;
}
bool GLFWFramework::setWindowIcon(Window *window, Texture *tex, GLenum target,
...
bool GLFWFramework::isWindowBorderless(Window *window) {
  return glfwGetWindowAttrib((GLFWwindow *)window->getHandle(),
                             GLFW_DECORATED) == GLFW_FALSE;
}
int GLFWFramework::getControllerID(void *handle) { return 0; }
bool GLFWFramework::isSameController(void *handle1, void *handle2) {
  return false;
}
void *GLFWFramework::openController(int id) { return nullptr; }
void GLFWFramework::closeController(void *handle) {}
bool GLFWFramework::isAttachedController(void *handle) { return false; }
...
void GLFWFramework::onError(int error, const char *description) {
  ...
  // LogManager::error(std::string("An error accoured: ") + to_string(error) +
  // std::string(": ") + std::string(description));
}

void GLFWFramework::onKey(GLFWwindow *window, int key, int scancode, int action,
                          int mods) {
  Event e;
  e.type =
      action == GLFW_PRESS
          ? EventType::KeyDown
          : (action == GLFW_RELEASE ? EventType::KeyUp : EventType::Undefined);
  e.key.type =
      action == GLFW_PRESS
          ? EventType::KeyDown
          : (action == GLFW_RELEASE ? EventType::KeyUp : EventType::Undefined);
...
void GLFWFramework::onMouseButton(GLFWwindow *window, int button, int action,
                                  int mods) {
  Event e;
  e.type = action == GLFW_PRESS
               ? EventType::MouseButtonDown
               : (action == GLFW_RELEASE ? EventType::MouseButtonUp
                                         : EventType::Undefined);
  e.button.button = button == GLFW_MOUSE_BUTTON_LEFT
                        ? Keys::MS_LEFT
                        : (button == GLFW_MOUSE_BUTTON_RIGHT ? Keys::MS_RIGHT
                                                             : Keys::MS_MIDDLE);
...
Keys GLFWFramework::toJohnnyKeys(int key) {
  ...
  case GLFW_KEY_KP_SUBTRACT:
    return Keys::KP_MEMSUBTRACT;
  case GLFW_KEY_KP_ADD:
    return Keys::KP_MEMADD;
