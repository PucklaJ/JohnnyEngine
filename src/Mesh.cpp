...
Mesh::Mesh()
{
	...
	if (m_vbo == 0)
	{
		SDL::LogManager::error("Couldn't create VertexBuffer for Mesh",true,true,false);
	}
	else
	{
		...
		if (m_ibo == 0)
		{
			SDL::LogManager::error("Couldn't create IndexBuffer for Mesh",true, true, false);
		}
		else
		{
			...
			if (m_vao == 0)
			{
				SDL::LogManager::error("Couldn't create VertexArrays for Mesh", true, true,false);
			}	
		}
	}
...
Mesh::Mesh(aiMesh* mesh,const aiScene* scene,bool switchZandY) : Mesh()
{
...
void Mesh::addVertices(Vertex* vertices, unsigned int size,GLuint* indices,unsigned int indexSize)
{
...
void Mesh::loadMaterial(SDL::ResourceManager* res)
{
