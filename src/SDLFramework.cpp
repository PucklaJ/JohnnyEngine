...
bool SDLFramework::initWindow() {
  ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1),
            "Couldn't enable double buffering: ");

  if (Settings::getb(SettingName::MSAA)) {
    ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, 1),
              "Couldn't enable Multisampling: ");
    ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES,
                                  Settings::geti(SettingName::MSAA_SAMPLES)),
              "Couldn't set the multisampling samples: ");
  }

  ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK,
                                SDL_GL_CONTEXT_PROFILE_CORE),
            "Couldn't set the profile mask: ");
  ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 3),
            "Couldn't set the context major version: ");
  ERROR_OUT(SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 3),
            "Couldn't set the context minor version: ");
...
bool SDLFramework::createOpenGLContext() {
  SDL_GLContext context = SDL_GL_CreateContext(
...
Window *SDLFramework::createWindow(const std::string &title, unsigned int x,
...
std::string SDLFramework::getError() { return std::string(SDL_GetError()); }
...
bool SDLFramework::setWindowFullscreen(Window *window, bool fullscreen,
                                       bool desktop) {
  if (SDL_SetWindowFullscreen((SDL_Window *)window->getHandle(),
                              fullscreen
...
bool SDLFramework::setWindowBorderless(Window *window, bool borderless) {
  SDL_SetWindowBordered((SDL_Window *)window->getHandle(),
                        borderless ? SDL_TRUE : SDL_FALSE);
  return true;
}
bool SDLFramework::setWindowIcon(Window *window, Texture *tex, GLenum target,
...
int SDLFramework::getControllerID(void *handle) {
  return SDL_JoystickInstanceID(
      SDL_GameControllerGetJoystick((SDL_GameController *)handle));
}
...
bool SDLFramework::isSameController(void *handle1, void *handle2) {
  return equal(SDL_JoystickGetGUID(
                   SDL_GameControllerGetJoystick((SDL_GameController *)handle1))
                   .data,
               SDL_JoystickGetGUID(
                   SDL_GameControllerGetJoystick((SDL_GameController *)handle2))
                   .data);
}
...
bool SDLFramework::isAttachedController(void *handle) {
  return SDL_GameControllerGetAttached((SDL_GameController *)handle) == SDL_TRUE
             ? true
             : false;
}

void SDLFramework::lockAndHideCursor() { SDL_SetRelativeMouseMode(SDL_TRUE); }
...
WindowEventType SDLFramework::toJohnnyWindowEventType(Uint8 id) {
  ...
  case SDL_WINDOWEVENT_SIZE_CHANGED:
    return WindowEventType::SizeChanged;
  case SDL_WINDOWEVENT_MINIMIZED:
    return WindowEventType::Minimized;
  case SDL_WINDOWEVENT_MAXIMIZED:
    return WindowEventType::Maximized;
  case SDL_WINDOWEVENT_RESTORED:
    return WindowEventType::Restored;
  case SDL_WINDOWEVENT_ENTER:
    return WindowEventType::Enter;
  case SDL_WINDOWEVENT_LEAVE:
    return WindowEventType::Leave;
  case SDL_WINDOWEVENT_FOCUS_GAINED:
    return WindowEventType::FocusGained;
  case SDL_WINDOWEVENT_FOCUS_LOST:
    return WindowEventType::FocusLost;
  case SDL_WINDOWEVENT_CLOSE:
    return WindowEventType::Close;
  case SDL_WINDOWEVENT_TAKE_FOCUS:
    return WindowEventType::TakeFocus;
  case SDL_WINDOWEVENT_HIT_TEST:
    return WindowEventType::HitTest;
  default:
