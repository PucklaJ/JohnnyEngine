...
#define FIND(_pair) (normalPosUVIndices.find(std::tuple<GLuint,GLuint,GLuint>(_pair.posIndex,_pair.normalIndex,_pair.uvIndex)))
#define INSERT(_pair) (normalPosUVIndices.insert(std::pair<std::tuple<GLuint,GLuint,GLuint>,GLuint>(std::tuple<GLuint,GLuint,GLuint>(_pair.posIndex,_pair.normalIndex,_pair.uvIndex), curIndex++)))
#define GET(_pair) (normalPosUVIndices[std::tuple<GLuint,GLuint,GLuint>(_pair.posIndex,_pair.normalIndex,_pair.uvIndex)])
...
const Material* getMaterial(const std::string& name,const std::vector<Material>& vec)
{
...
void OBJLoader::load()
{
			...
			else if (tokens[0] == "f")
			{
				...
				if (n1 == std::string::npos)
				{
					...
					else
					{
						...
						pair[1].posIndex = atoi(tokens[2].substr(0, tokens[2].find('/', 0)).c_str()) - 1;
						pair[2].posIndex = atoi(tokens[3].substr(0, tokens[3].find('/', 0)).c_str()) - 1;

						n2 = tokens[1].find('/',n1+1);
						pair[0].uvIndex = atoi(tokens[1].substr(n1+1,n2-n1-1).c_str()) - 1;
						n2 = tokens[2].find('/',0);
						n3 = tokens[2].find('/',n2+1);
						pair[1].uvIndex = atoi(tokens[2].substr(n2+1,n3-n2-1).c_str()) - 1;
						n2 = tokens[3].find('/',0);
						n3 = tokens[3].find('/',n2+1);
						pair[2].uvIndex = atoi(tokens[3].substr(n2+1,n3-n2-1).c_str()) - 1;

						pair[0].normalIndex = atoi(tokens[1].substr(tokens[1].find_last_of('/')+1).c_str()) - 1;
						pair[1].normalIndex = atoi(tokens[2].substr(tokens[2].find_last_of('/')+1).c_str()) - 1;
	...
	for (size_t i = 0;i<normalPosUVIndicesAll.size();i++)
	{
		...
		if (normalPosUVIndicesAll[i].material != "NONE" && (lastMaterial.name == "NONE" || lastMaterial.name != normalPosUVIndicesAll[i].material))
		{
			const Material* pointer = getMaterial(normalPosUVIndicesAll[i].material, materials);
			if (pointer)
...
bool OBJLoader::findInMap(std::map<std::tuple<GLuint, GLuint, GLuint>, GLuint>& _map, const std::tuple<GLuint, GLuint, GLuint>& key)
{
	for (std::map<std::tuple<GLuint, GLuint, GLuint>, GLuint>::iterator it = _map.begin(); it != _map.end(); it++)
	{
		if (std::get<0>(it->first) == std::get<0>(key) && std::get<1>(it->first) == std::get<1>(key) && std::get<2>(it->first) == std::get<2>(key))
		{
			return true;
		}
	}
...
void MTLLoader::load()
{
	...
	if (fileReader.fail())
	{
		std::cerr << std::endl << "Failed to open MaterialFile: " << m_fileName << std::endl;
		return;
	}
		...
		else
		{
			if (fileContents[i] == ' ')
			{
				if (curToken == "newmtl")
				{
					if (readingMaterial)
					{
						materialFileContents.push_back(curMaterialContents.substr(0, curMaterialContents.length() - curToken.length()));
					}
...
Material::Material() :
	...
	emittedLight(0.0f, 0.0f, 0.0f),
	specularExponent(1.0f),
	transparency(1.0f),
	transmissionFilter(1.0f, 1.0f, 1.0f),
	illumination(0),
	ambientTexture(""),
	diffuseTexture(""),
	specularTexture(""),
	ambientTextureT(nullptr),
	diffuseTextureT(nullptr),
...
void Material::loadTextures(SDL::ResourceManager* res)
{
	if (ambientTexture != "")
	{
