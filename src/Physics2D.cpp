...
namespace Johnny {
Physics2D::Physics2D(MainClass *mainClass, const Vector2f &gravity,
...
void Physics2D::quit() { delete m_world; }
...
float Physics2D::scalarPixelToWorld(float d) { return d / m_pixelsPerMetre; }

float Physics2D::scalarWorldToPixel(float d) { return d * m_pixelsPerMetre; }

b2Body *Physics2D::createBOX(const Vector2f &pos, float w, float h,
...
b2Body *Physics2D::createCIRCLE(const Vector2f &pos, float radius,
...
b2Body *Physics2D::createCAPSULE(const Vector2f &pos, float w, float h,
                                 float friction, float restitution,
                                 float density, b2BodyType type) {
  b2BodyDef bdef;
  b2FixtureDef fdef;
