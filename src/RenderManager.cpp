...
void RenderManager::loadDefaultShaders(ResourceManager *res) {
  if (DEFAULT_POST_PROCESSING_SHADER == nullptr) {
    ...
    DEFAULT_POST_PROCESSING_SHADER->addVertexShader(
        res->loadShader("vertexShaderFrameBuffer.glsl"));
    DEFAULT_POST_PROCESSING_SHADER->addFragmentShader(
        res->loadShader("fragmentShaderFrameBuffer.glsl"));
...
void RenderManager::loadDefaultShaders3D(ResourceManager *res,
                                         Lighting3D *light) {
  if (DEFAULT_SHADER == nullptr) {
    DEFAULT_SHADER = new Shader();
    DEFAULT_SHADOWMAP_SHADER = new Shader();

    std::map<std::string, std::string> defineChanges =
        light->getDefineChanges();

    DEFAULT_SHADER->addVertexShader(res->loadShader("vertexShader.glsl"));
    DEFAULT_SHADER->addFragmentShader(
        res->loadShader("fragmentShader.glsl", &defineChanges));

    DEFAULT_SHADOWMAP_SHADER->addVertexShader(
        res->loadShader("vertexShaderShadowMap.glsl"));
    DEFAULT_SHADOWMAP_SHADER->addFragmentShader(
        res->loadShader("fragmentShaderShadowMap.glsl"));
...
void RenderManager::removeShader(Shader *s) { m_shaderActors.erase(s); }
...
void RenderManager::render(MainClass *m) {
  ...
  for (std::map<Shader *, std::vector<Actor *>>::iterator it =
           m_shaderActors.begin();
       it != m_shaderActors.end(); it++) {
    actors = &it->second;
    shader = it->first;
    shader->bind();
